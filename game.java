...
public class game extends JFrame implements ActionListener {
	    ...
	    //public String rmsg; //서버로부터 받은 메시지
	    ...
	    public game() {
	        ...
	        textArea_1.setBounds(978, 22, 274, 301);
	...
	class game_Frame extends JPanel implements KeyListener, Runnable {
	    ...
	    private void showGameOverPopup(String winner) {
	        ...
	        if (result == JOptionPane.YES_OPTION) {
	        	...
	        	/*try {
                    os.writeUTF("restartGame"); // 재시작 요청 알리기
                } catch (IOException ex) {
                    ex.printStackTrace();
                }*/
	            restartGame(); // 게임 재시작
	    ...
	    private void restartGame() {
	    	synchronized (this) {
	            Thread restartThread = new Thread(() -> {
	                while (true) {
	                    // 상대방이 재시작을 수락할 때까지 대기
	                    JOptionPane.showMessageDialog(null, "상대방의 재시작을 기다리는 중 입니다.", "알림", JOptionPane.INFORMATION_MESSAGE);
	                    // 여기서 reGame 값이 true가 되면 루프를 종료
	                    if (reGame) {
	                        break;
	                    }
	                }
	            ...
	            Thread thread2 = new Thread(() -> {
	                while (true) {
	                    try {
	                        ...
	                        /*
	                        else if (messageType.equals("restartGame")) {
	                            // 상대방의 재시작 수신하기
	                        	reGame = true;
	    ...
	    private void displayMessage(String message) {
	        // textArea를 업데이트하고자 하는 코드를 이 메서드에 추가
	        // SwingUtilities.invokeLater를 사용하여 UI 업데이트가 이루어지도록 합니다.
	        SwingUtilities.invokeLater(() -> {
